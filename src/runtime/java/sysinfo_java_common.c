...
// Extern — implemented by runtime-specific file (sysinfo_openjdk.c, sysinfo_graalvm.c, etc.)
...
// Print logo and info — shared across all Java family runtimes
void sysinfo_print(sysinfo_putline_fn putline, void *ctx) {
    ...
    char threads_line[64], classloader_line[64];
    ...
    snprintf(classloader_line,sizeof(classloader_line), "Environment:     %s", st.environment);

    const char *info[] = {
        header, separator, impl_line, ver_line,
        uptime_line, heap_line, gc_line,
        threads_line, classloader_line
    };
